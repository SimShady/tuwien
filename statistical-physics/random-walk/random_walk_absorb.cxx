...
int main() {
  ...
  ofstream expectation_values("data/expectation_values_absorb.dat");
  ofstream point_distribution("data/point_distribution_absorb.dat");
  ofstream expectation_values_random_step("data/expectation_values_random_step_absorb.dat");
  ofstream point_distribution_random_step("data/point_distribution_random_step_absorb.dat");
  for(size_t i = 1; i <= iterations; i++) {
    ...
    for(size_t n = 0; n < N; n++) {
      ...
      if (points[n]+iterations/2 < point_dist.size()) {
        point_dist[points[n]+iterations/2] -= 1;
      }
      if (static_cast<int>(floor(points_random_step[n]))+iterations/2 < point_dist_random_step.size()) {
      ...
      if(points[n] > 50.0 || points[n] < -50.0) {
        points[n] = 100000.0;
      }
      if (points_random_step[n] > 50.0 || points_random_step[n] < -50.0) {
        points_random_step[n] = 100000.0;
      }
      if (points[n]+iterations/2 < point_dist.size()) {
        point_dist[points[n]+iterations/2] += 1;
      }
